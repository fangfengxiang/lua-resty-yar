...
--- config
    ...
    location /t {
        content_by_lua_block {
            -- serve_callback checks #data > max_body_len (100), so 100 bytes is accepted.
            -- handle_message tries to parse, fails (not valid YAR), returns YAR error response (s=1) with HTTP 200.
