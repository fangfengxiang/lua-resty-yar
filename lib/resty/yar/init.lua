...
--   2. 进程级 Server / TcpServer 实例创建（方法表 memoize，worker 内共享）
--   3. 配置合并（连接级超时、保活等参数）
--
--   http {
...
function _M.setup(opts)
    ...
    -- 合并用户配置（service 和 on_worker_init 不混入 config）
...
-- @param opts table|nil per-client 选项覆盖（timeout/packager/provider/token 等）
